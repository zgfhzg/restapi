...
ENTRYPOINT ["java", "-DDATABASE_URL=${DB_URL}", "-DDATABASE_PORT=${DB_PORT}", "-DDATABASE_NAME=${DB_NAME}", "-DDATABASE_USERNAME=${DB_USERNAME}", "-DDATABASE_PASSWORD=${DB_PASSWORD}", "-DEUREKA_URI=${EUREKA}", "-DSERVER_DNS=${DNS}", "-jar", "/restapi/app.jar"]